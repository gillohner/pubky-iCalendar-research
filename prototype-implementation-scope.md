...
### Research and Feature Mapping (1-2 weeks)

- Get familiar with existing Pubky Apps
- Research and document existing iCalendar standards
- Scope prototype
- Define Nexus endpoints and changes to existing endpoints
- Define new homeserver documents and structure

### Integrate Nexus Endpoints and changes to existing endpoints (~2-3 weeks)

- Decide on implementation approach (PubkyAppPostKind or new types)
...
- Add new Pubky app types or PubkyAppPostKind enum values
...
```
Note: Personal unfamiliarity with Rust and Nexus makes it hard to know how realistic this scope is. It could potentially be shorter, but making sure the integration is robust is a priority.
```
...
- Integrate OSM-tag-based location indexing in Nexus for events
...
| Name                     | Description                                                                                                                                                                                                                                                                  |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Calendar/Event Header    | Header for a calendar wi1th image, summary, host, admins, categories, etc. Subcomponents are dynamically rendered based on whether it's a calendar/event and what metadata is present                                                                                        |
...
| Attendees                | Component rendering attendees of an event with their respective status. Inviting attendees to an event will not be implemented yet in event creation. Could be added with existing tags in the future. Supports per-instance RSVPs for recurring events via `recurrence_id`. |
